...
func ValidateMovie(v *validator.Validator, movie *Movie) {
	...
	v.Check(len(movie.Genres) >= 1, "genres", "must contain at leats 1 genre")
...
func (m MovieModel) GetAll(title string, genres []string, filters Filters) ([]*Movie, Metadata, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&totalRecords, &movie.ID, &movie.CreatedAt, &movie.Title, &movie.Year, 
	...
	metadata := calculatedMetadata(totalRecords, filters.Page, filters.PageSize)
